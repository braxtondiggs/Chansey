...
export class ShutdownService implements OnApplicationBootstrap, OnApplicationShutdown {
  ...
  constructor(
    ...
    @InjectQueue('exchange-health-queue') private readonly exchangeHealthQueue: Queue,
    @InjectQueue('exchange-queue') private readonly exchangeQueue: Queue,
    @InjectQueue('listing-announcement-poll') private readonly listingAnnouncementPollQueue: Queue,
    @InjectQueue('listing-score') private readonly listingScoreQueue: Queue,
    @InjectQueue('listing-time-stop') private readonly listingTimeStopQueue: Queue,
    @InjectQueue('listing-trade-execution') private readonly listingTradeExecutionQueue: Queue,
  ...
  private get queues(): { name: string; queue: Queue }[] {
    return [
      ...
      { name: 'exchange-health-queue', queue: this.exchangeHealthQueue },
      { name: 'exchange-queue', queue: this.exchangeQueue },
      { name: 'listing-announcement-poll', queue: this.listingAnnouncementPollQueue },
      { name: 'listing-score', queue: this.listingScoreQueue },
      { name: 'listing-time-stop', queue: this.listingTimeStopQueue },
      { name: 'listing-trade-execution', queue: this.listingTradeExecutionQueue },
      ...
      { name: 'coin-selection-queue', queue: this.coinSelectionQueue },
