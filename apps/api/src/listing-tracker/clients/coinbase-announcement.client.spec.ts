...
import { CoinbaseAnnouncementClient } from './coinbase-announcement.client';
...
const LAST_SEEN_KEY = 'listing-tracker:last-seen:coinbase';
...
const SAMPLE_PRODUCTS = [
  ...
  { id: 'LEGACY-USD', base_currency: 'LEGACY', quote_currency: 'USD', status: 'delisted' },
  { id: 'EUR-USD', base_currency: 'EUR', quote_currency: 'USD', status: 'online', fx_stablecoin: true },
  { id: 'FOO-EUR', base_currency: 'FOO', quote_currency: 'EUR', status: 'online' },
  { id: 'BTC-USDC', base_currency: 'BTC', quote_currency: 'USDC', status: 'online' }
];

describe('CoinbaseAnnouncementClient', () => {
  let circuitBreaker: CircuitBreakerService;
  let redis: RedisMock;
  let client: CoinbaseAnnouncementClient;
  let fetchSpy: jest.SpyInstance;

  beforeEach(() => {
    circuitBreaker = new CircuitBreakerService();
    redis = makeRedisMock();
    client = new CoinbaseAnnouncementClient(circuitBreaker, redis as unknown as Redis);
  ...
  describe('getLatest', () => {
    it('filters to online products with USD/USDC quote, excluding fx_stablecoin', async () => {
      fetchSpy.mockResolvedValue(makeFetchResponse(SAMPLE_PRODUCTS));
    ...
    it('emits stable synthetic externalIds and source URLs for each base', async () => {
      fetchSpy.mockResolvedValue(makeFetchResponse([SAMPLE_PRODUCTS[0]]));

      const [announcement] = await client.getLatest();

      expect(announcement).toMatchObject({
        exchangeSlug: 'coinbase',
        externalId: 'coinbase-listing:BTC',
        sourceUrl: 'https://exchange.coinbase.com/markets/BTC-USD',
        announcedSymbol: 'BTC',
    ...
    it('dedupes bases when both USD and USDC pairs exist for the same coin', async () => {
    ...
    it('records circuit-breaker failure and throws on HTTP error', async () => {
      ...
      expect(recordFailureSpy).toHaveBeenCalledWith('listing-tracker:coinbase');
    });

    it('throws CircuitOpenError when circuit is open without calling fetch', async () => {
      for (let i = 0; i < 5; i++) circuitBreaker.recordFailure('listing-tracker:coinbase');
  ...
  describe('onModuleInit bootstrap seeding', () => {
    it('seeds the poller last-seen set and sentinel when sentinel is absent', async () => {
      ...
      expect(redis.__pipeline.sadd).toHaveBeenCalledWith(LAST_SEEN_KEY, 'coinbase-listing:BTC');
      expect(redis.__pipeline.sadd).toHaveBeenCalledWith(LAST_SEEN_KEY, 'coinbase-listing:ETH');
