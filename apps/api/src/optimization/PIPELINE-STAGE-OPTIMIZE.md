...
The OPTIMIZE stage finds optimal strategy parameters using walk-forward analysis, which helps prevent overfitting by validating parameters on out-of-sample data.
...
```typescript
compositeScore = w1 * sharpe + w2 * totalReturn + w3 * maxDrawdown + w4 * winRate
```
...
const run = await optimizationService.startOptimization(
  strategyConfigId,
  parameterSpace,
  {
    method: 'grid_search',
