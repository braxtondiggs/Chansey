...
export class ExchangeKeyHealthService implements OnModuleInit {
  ...
  async checkKeyHealth(exchangeKey: ExchangeKey): Promise<void> {
    ...
    try {
      ...
      user = await this.usersService.getById(exchangeKey.userId);
      if (!user) {
        this.logger.warn(`User ${exchangeKey.userId} not found for exchange key ${exchangeKey.id}, skipping`);
        return;
      }
    ...
    await this.healthLogRepo.save(
      this.healthLogRepo.create({
        ...
        status: exchangeKey.healthStatus,
        errorCategory,
        errorMessage,
        responseTimeMs
      })
