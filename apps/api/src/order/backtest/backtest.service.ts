  ...
  async ensureDefaultDatasetExists(): Promise<MarketDataSet | null> {
    try {
      ...
      const [coinIds, dateRange] = await Promise.all([
        this.priceService.getCoinsWithPriceData(),
        this.priceService.getPriceDataDateRange()
      ]);

      if (coinIds.length === 0 || !dateRange) {
        this.logger.debug('No price data available - skipping default dataset creation');
        return null;
      }
      ...
      await this.marketDataSetRepository.upsert(
        {
          ...
          integrityScore: 90,
          checksum,
          storageLocation: '', // Empty string = use database Price table (see MarketDataReaderService.hasStorageLocation)
          replayCapable: false,
