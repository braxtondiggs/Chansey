  ...
  async sendExchangeKeyWarningEmail(
    ...
    return this.sendEmail({
      ...
      html: wrapTemplate({
        ...
        body: `
          <p style="${bodyText()}">Hi ${escapeHtml(name)},</p>
          ${alertBox(COLORS.warning, `Your <strong>${escapeHtml(exchangeName)}</strong> API key has failed ${failures} consecutive health checks.`)}
  ...
  async sendExchangeKeyDeactivatedEmail(email: string, name: string, exchangeName: string): Promise<boolean> {
    ...
    return this.sendEmail({
      ...
      html: wrapTemplate({
        ...
        body: `
          <p style="${bodyText()}">Hi ${escapeHtml(name)},</p>
          ${alertBox(COLORS.danger, `Your <strong>${escapeHtml(exchangeName)}</strong> API key has been automatically deactivated after repeated authentication failures.`)}
...
function alertBox(color: string, message: string): string {
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin: 16px 0 20px 0;">
  <tr>
    <td style="background-color: ${color}10; border-left: 4px solid ${color}; border-radius: 0 8px 8px 0; padding: 14px 18px;">
