...
<!-- Simple error state -->
@if (query.isError()) {
<p-message severity="error" [text]="query.error()?.message || 'An error occurred'" styleClass="w-full" />
}
```
...
>
  ...
  <ng-template pTemplate="header">
    <tr class="whitespace-nowrap">
      <th pSortableColumn="name" class="min-w-48">Name <p-sortIcon field="name" /></th>
      <th pSortableColumn="value" class="min-w-32 text-right">Value <p-sortIcon field="value" /></th>
    </tr>
  </ng-template>
...
<p-button type="submit" [loading]="mutation.isPending()" [raised]="true" [style]="{ width: '100%' }">
  <span>Submit</span>
</p-button>
...
<p-button type="button" [text]="true" [loading]="loading()" (click)="onClick()">
...
<form [formGroup]="form" (ngSubmit)="onSubmit()">
  ...
  <p-floatlabel variant="on" class="mt-4">
    <input type="text" formControlName="email" pInputText class="w-full" pSize="large" required autocomplete="email" />
    <label>Email</label>
  </p-floatlabel>
...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
  <div
    class="border-surface-border bg-surface-card rounded-lg border p-6 transition-transform hover:-translate-y-0.5 hover:shadow-lg"
  >
    <div class="text-color-secondary mb-2 text-sm font-medium">Market Cap</div>
    <div class="flex items-baseline gap-1 text-2xl font-bold text-color">
      <span>$1.2T</span>
    </div>
    <div class="text-primary-color mt-2 text-xs font-semibold">Rank #1</div>
  </div>
</div>
```
...
```html
<!-- Price change tag -->
<p-tag [severity]="change >= 0 ? 'success' : 'danger'" [value]="change" />

<!-- Helper function for severity -->
getTag(change: number | undefined): 'success' | 'danger' { if (change === undefined) return 'success'; return +change >=
0 ? 'success' : 'danger'; }
```
...
```html
<p-avatar [image]="item.image" shape="circle" size="large" styleClass="min-w-10 shadow-sm" />
```
...
<div class="flex min-h-screen items-center justify-center">
  ...
  <div class="flex items-center gap-3">
    <!-- Card styling -->
    <div class="border-surface-border bg-surface-card rounded-lg border p-6 shadow-sm">
      <!-- Overflow handling for tables -->
      <div class="overflow-x-auto">
        <div class="min-w-[800px] md:min-w-0"></div>
      </div>
    </div>
  </div>
</div>
...
```html
<!-- Fade in animation -->
<section class="animate-fadein animate-duration-300 animate-ease-in"></section>
```
...
{{ value | currency }} {{ value | currency:'USD':'symbol':'1.2-2' }}
...
<div class="space-y-1">
  ...
  <p-progressBar [value]="(circulating / maxSupply) * 100" [showValue]="false" styleClass="!h-2" />
</div>
}
```
...
<input pInputText id="email" formControlName="email" required autocomplete="email" />
<small id="email-error" class="text-red-500"> Email is required </small>
```
...
| Tailwind Config   | `apps/chansey/src/tailwind.css`           |
